// src/app/api/nfl/calculate/route.ts
...
import {
  ...
  getProcessedNflGameIds,
  loadNflConfig,
  loadNflRatings,
  loadLinedNflClosingLines,
  saveNflAdjustment,
  saveNflClosingLine,
  saveNflConfig,
  upsertNflRatings,
} from '@/lib/nfl/supabase';
import { replayLedger } from '@/lib/nfl/replay';
import { matchOddsEvent } from '@/lib/nfl/teamNames';
import { EspnNflGame, NflTeamRating } from '@/lib/nfl/types';
...
async function fetchNflGamesForDate(dateYmd: string): Promise<EspnNflGame[]> {
  const url = `${ESPN_NFL_SCOREBOARD_URL}?dates=${dateYmd}&limit=100`;
...
function buildLookups(ratings: Map<string, NflTeamRating>) {
  const byEspnId = new Map<string, NflTeamRating>();
  const byEspnName = new Map<string, NflTeamRating>();
...
async function handleSync(body: {
  ...
  const config = await loadNflConfig();
  const season = body.season ?? config.season ?? NFL_SEASON;
  const seasonDates = NFL_SEASON_DATES[season];
  if (!seasonDates) throw new Error(`No season dates configured for ${season}`);

  const ratings = await loadNflRatings(season);
  if (ratings.size === 0) {
    throw new Error('nfl_ratings is empty — run the market seed first (POST /api/nfl/seed)');
  }
  const { byEspnId, byEspnName } = buildLookups(ratings);
  const processedIds = await getProcessedNflGameIds(season);

  // Scan back NFL_SYNC_LOOKBACK_DAYS from the last processed game. Processed
  ...
  const windowStart =
    body.fullScan || !config.lastProcessedDate
      ? seasonDates.start
      : [shiftYmd(config.lastProcessedDate, -NFL_SYNC_LOOKBACK_DAYS), seasonDates.start]
          .sort()
          .pop()!;
  ...
  const maxLedgerDate = await getMaxNflAdjustmentDate(season);
  const pendingLined = (await loadLinedNflClosingLines()).filter(
    (l) =>
  ...
  outer: for (const day of dateRange(startDate, endDate)) {
    ...
    for (const game of games) {
      ...
      if (!home || !away) {
        // NFL opponent, non-D1 opponent, or unlinked team — not part of the NFL pool
        skipped.push({
          game: label,
          date: day,
      ...
      if (cached && !(cached.closingSpread === null && body.retryMissing)) {
        ...
        const freeze = new Date(
          new Date(game.date).getTime() - NFL_CLOSING_TIME_MINUTES * 60 * 1000
        )
          .toISOString()
          .replace(/\.\d{3}Z$/, 'Z');
        ...
        if (match) {
          ...
          const consensus = extractConsensusSpread(match.event, NFL_CONSENSUS_BOOKS);
          if (consensus) {
  ...
  await saveNflConfig({ ...config, season, lastProcessedDate: lastGameDate });
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('nfl/calculate failed:', e);
