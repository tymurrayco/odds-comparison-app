// src/app/api/nfl/manual-adjustment/route.ts
...
import {
  deleteNflManualAdjustment,
  insertNflManualAdjustment,
  loadNflRatings,
  updateNflManualAdjustment,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await insertNflManualAdjustment({
      teamName,
      season,
      adjustDate,
      delta,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await updateNflManualAdjustment(id, fields);
    const replay = await replayOr(body.season ?? NFL_SEASON);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const id = Number(body.id);
    if (!Number.isFinite(id)) throw new Error('id is required');
    await deleteNflManualAdjustment(id);
    const replay = await replayOr(body.season ?? NFL_SEASON);
    return NextResponse.json({ success: true, id, replay });
  } catch (e) {
    return NextResponse.json(
