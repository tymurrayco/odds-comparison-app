// src/app/api/fcs/manual-adjustment/route.ts
...
import {
  deleteFcsManualAdjustment,
  insertFcsManualAdjustment,
  loadFcsRatings,
  updateFcsManualAdjustment,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await insertFcsManualAdjustment({
      teamName,
      season,
      adjustDate,
      delta,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await updateFcsManualAdjustment(id, fields);
    const replay = await replayOr(body.season ?? FCS_SEASON);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const id = Number(body.id);
    if (!Number.isFinite(id)) throw new Error('id is required');
    await deleteFcsManualAdjustment(id);
    const replay = await replayOr(body.season ?? FCS_SEASON);
    return NextResponse.json({ success: true, id, replay });
  } catch (e) {
    return NextResponse.json(
