...
// Applies to the engines and to the ledger replay, so a Recalculate All
// strips the effect of earlier over-cap games. The game is still written to
// the ledger with adjustment 0 (so it is not retried) and does not count
// toward games_processed.
//
// College tightened 2026-09-25 (Tyler): FBS and FCS closing lines of 30 or
// more do not re-rate. NFL keeps the ±35 rule.
...
export const CFB_MAX_RATED_SPREAD = 30;

export function exceedsCfbRatedSpreadCap(closingSpread: number): boolean {
  return Math.abs(closingSpread) >= CFB_MAX_RATED_SPREAD;
}
