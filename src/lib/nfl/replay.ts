// src/lib/nfl/replay.ts

/**
 * Ledger replay: reset every team to its market seed and re-apply the season's
...
import { projectNflSpread, roundToDecimal } from './engine';
import {
  loadNflAdjustments,
  loadNflManualAdjustments,
  loadNflRatings,
  saveNflAdjustment,
  stampNflManualAdjustment,
  upsertNflRatings,
} from './supabase';
import { NflGameAdjustment, NflManualAdjustment } from './types';
import { exceedsRatedSpreadCap } from '@/lib/ratedSpreadCap';

type ReplayEvent =
  | { kind: 'game'; date: string; order: number; adj: NflGameAdjustment }
  | { kind: 'manual'; date: string; order: number; manual: NflManualAdjustment };
...
export async function replayLedger(
  ...
  for (const ev of events) {
    if (ev.kind === 'manual') {
      ...
      await stampNflManualAdjustment(ev.manual.id, before, team.rating);
      manualApplied++;
      continue;
    }
    ...
    // (a seed metadata refresh must not silently shift the whole ledger).
    const hfaApplied = adj.hfaApplied;
    const projected = projectNflSpread(home.rating, away.rating, hfaApplied);
  ...
  await upsertNflRatings([...ratings.values()]);
