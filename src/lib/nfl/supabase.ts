// src/lib/nfl/supabase.ts

/**
 * Supabase I/O for the NFL ratings system.
 * Tables: nfl_ratings, nfl_game_adjustments, nfl_closing_lines, nfl_ratings_config
 * (DDL in sql/nfl_ratings.sql).
 */

import { createClient, SupabaseClient } from '@supabase/supabase-js';
import { NFL_DEFAULT_HFA, NFL_SEASON } from './constants';
import {
  NflClosingLine,
  NflConfig,
  NflGameAdjustment,
...
function toRating(r: RatingRow): NflTeamRating {
  return {
...
export async function loadNflRatings(
  season: number = NFL_SEASON
): Promise<Map<string, NflTeamRating>> {
  const { data, error } = await getClient()
    ...
    .range(0, PAGE - 1);
  if (error) throw new Error(`loadNflRatings: ${error.message}`);
  const map = new Map<string, NflTeamRating>();
...
export async function upsertNflRatings(ratings: NflTeamRating[]): Promise<void> {
  if (ratings.length === 0) return;
  const rows = ratings.map((r) => ({
    team_name: r.teamName,
    seed_name: r.seedName,
    espn_name: r.espnName,
    espn_id: r.espnId,
    espn_abbr: r.espnAbbr,
    conference: r.conference,
    rating: r.rating,
  ...
  for (let i = 0; i < rows.length; i += 100) {
    ...
    if (error) throw new Error(`upsertNflRatings: ${error.message}`);
  }
}
...
function toAdjustment(r: AdjustmentRow): NflGameAdjustment {
  return {
    gameId: r.game_id,
...
export async function getProcessedNflGameIds(
  season: number = NFL_SEASON
): Promise<Set<string>> {
  const ids = new Set<string>();
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await getClient()
      .from('nfl_game_adjustments')
      .select('game_id')
      .eq('season', season)
      .range(from, from + PAGE - 1);
    if (error) throw new Error(`getProcessedNflGameIds: ${error.message}`);
...
export async function loadNflAdjustments(
  season: number = NFL_SEASON
): Promise<NflGameAdjustment[]> {
  const out: NflGameAdjustment[] = [];
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await getClient()
      .from('nfl_game_adjustments')
      .select('*')
      ...
      .range(from, from + PAGE - 1);
    if (error) throw new Error(`loadNflAdjustments: ${error.message}`);
...
export async function getMaxNflAdjustmentDate(
  season: number = NFL_SEASON
): Promise<string | null> {
  const { data, error } = await getClient()
    .from('nfl_game_adjustments')
    ...
    .limit(1);
  if (error) throw new Error(`getMaxNflAdjustmentDate: ${error.message}`);
  return data?.[0]?.game_date?.substring(0, 10) ?? null;
}

export async function saveNflAdjustment(adj: NflGameAdjustment): Promise<void> {
  const { error } = await getClient()
    .from('nfl_game_adjustments')
    .upsert(
      {
  ...
  if (error) throw new Error(`saveNflAdjustment(${adj.gameId}): ${error.message}`);
}

// ---------- closing line cache ----------

export async function getCachedNflClosingLine(
  gameId: string
): Promise<NflClosingLine | null> {
  const { data, error } = await getClient()
    .from('nfl_closing_lines')
    .select('*')
    .eq('game_id', gameId)
    .limit(1);
  if (error) throw new Error(`getCachedNflClosingLine: ${error.message}`);
  const row = data?.[0];
  if (!row) return null;
...
function toClosingLine(row: any): NflClosingLine {
  return {
    gameId: row.game_id,
...
export async function loadUnlinedNflGames(limit = 100): Promise<NflClosingLine[]> {
  const { data, error } = await getClient()
    .from('nfl_closing_lines')
    .select('*')
    ...
    .order('game_date', { ascending: false })
    .limit(limit);
  if (error) throw new Error(`loadUnlinedNflGames: ${error.message}`);
...
export async function loadLinedNflClosingLines(): Promise<NflClosingLine[]> {
  const out: NflClosingLine[] = [];
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await getClient()
      .from('nfl_closing_lines')
      .select('*')
      ...
      .order('game_date', { ascending: true })
      .range(from, from + PAGE - 1);
    if (error) throw new Error(`loadLinedNflClosingLines: ${error.message}`);
...
export async function setManualNflClosingLine(
  gameId: string,
  closingSpread: number
): Promise<NflClosingLine> {
  const { data, error } = await getClient()
    .from('nfl_closing_lines')
    .update({
    ...
    .eq('game_id', gameId)
    .select();
  if (error) throw new Error(`setManualNflClosingLine: ${error.message}`);
...
export async function saveNflClosingLine(line: NflClosingLine): Promise<void> {
  const { error } = await getClient()
    .from('nfl_closing_lines')
    .upsert(
      {
  ...
  if (error) throw new Error(`saveNflClosingLine(${line.gameId}): ${error.message}`);
}

// ---------- manual rating adjustments ----------

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function toManual(row: any): NflManualAdjustment {
  return {
    id: row.id,
...
export async function loadNflManualAdjustments(
  season: number = NFL_SEASON
): Promise<NflManualAdjustment[]> {
  const { data, error } = await getClient()
    .from('nfl_manual_adjustments')
    .select('*')
    .eq('season', season)
  ...
  if (error) {
    ...
    throw new Error(`loadNflManualAdjustments: ${error.message}`);
  }
  return (data ?? []).map(toManual);
}

export async function insertNflManualAdjustment(m: {
  teamName: string;
  season: number;
  adjustDate: string;
  delta: number;
  ...
  const { data, error } = await getClient()
    ...
    .select();
  if (error) throw new Error(`insertNflManualAdjustment: ${error.message}`);
  return toManual(data![0]);
}

export async function updateNflManualAdjustment(
  id: number,
  ...
  const { data, error } = await getClient()
    ...
    .eq('id', id)
    .select();
  if (error) throw new Error(`updateNflManualAdjustment: ${error.message}`);
  if (!data || data.length === 0) throw new Error(`Manual adjustment ${id} not found`);
}

export async function deleteNflManualAdjustment(id: number): Promise<void> {
  const { error } = await getClient()
    .from('nfl_manual_adjustments')
    .delete()
    .eq('id', id);
  if (error) throw new Error(`deleteNflManualAdjustment: ${error.message}`);
}

/** Stamp replay results onto a manual adjustment row. */
export async function stampNflManualAdjustment(
  id: number,
  ratingBefore: number,
  ratingAfter: number
): Promise<void> {
  const { error } = await getClient()
    ...
    .eq('id', id);
  if (error) throw new Error(`stampNflManualAdjustment: ${error.message}`);
}

// ---------- config ----------

export async function loadNflConfig(): Promise<NflConfig> {
  const { data, error } = await getClient()
    .from('nfl_ratings_config')
    .select('*')
    .eq('id', 1)
    .limit(1);
  if (error) throw new Error(`loadNflConfig: ${error.message}`);
  const row = data?.[0];
  if (!row) {
    return {
      hfaDefault: NFL_DEFAULT_HFA,
...
export async function saveNflConfig(config: NflConfig): Promise<void> {
  const { error } = await getClient()
    .from('nfl_ratings_config')
    .upsert(
      {
  ...
  if (error) throw new Error(`saveNflConfig: ${error.message}`);
}
