// src/lib/fbs/replay.ts

/**
 * Ledger replay: reset every team to its Brad Powers seed and re-apply the season's
...
import { projectFbsSpread, roundToDecimal } from './engine';
import {
  loadFbsAdjustments,
  loadFbsManualAdjustments,
  loadFbsRatings,
  saveFbsAdjustment,
  stampFbsManualAdjustment,
  upsertFbsRatings,
} from './supabase';
...
type ReplayEvent =
  ...
  | { kind: 'manual'; date: string; order: number; manual: FbsManualAdjustment };
...
export async function replayLedger(
  ...
  for (const ev of events) {
    if (ev.kind === 'manual') {
      ...
      await stampFbsManualAdjustment(ev.manual.id, before, team.rating);
      manualApplied++;
      continue;
    }
  ...
  await upsertFbsRatings([...ratings.values()]);
