...
interface RatingsResponse {
  ...
  season: number;
  config?: FbsConfig;
  ratings: FbsTeamRating[];
  adjustments: FbsGameAdjustment[];
  totalAdjustments: number;
  unlinedGames: FbsClosingLine[];
  manualAdjustments: FbsManualAdjustment[];
  pendingManualCount: number;
  error?: string;
}

type HistoryEntry =
  | { kind: 'game'; date: string; adj: FbsGameAdjustment; isHome: boolean }
  | { kind: 'manual'; date: string; m: FbsManualAdjustment };
        ...
        <div className="text-sm font-medium text-slate-800 truncate group-hover/team:underline">
          ...
          {warn && (
            <span
              ...
              title="No ESPN link — games for this team will be skipped. Add an override in src/lib/fbs/teamNames.ts and re-seed."
            >
              ⚠
            </span>
          )}
...
export default function FbsRatingsView({ admin = false }: { admin?: boolean }) {
  ...
  const [view, setView] = useState<'ratings' | 'upcoming' | 'futures' | 'sos'>(admin ? 'ratings' : 'upcoming');
  ...
  // Shared tab link: ?view=ratings|upcoming|futures|sos (read once; mirrored below)
  useEffect(() => {
    const v = new URLSearchParams(window.location.search).get('view');
    if (v === 'ratings' || v === 'upcoming' || v === 'futures' || v === 'sos') setView(v);
  }, []);
  useEffect(() => {
  ...
  const loadUpcoming = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/fbs/upcoming?days=14');
  ...
  const submitBet = async (g: UpcomingGame) => {
    ...
    try {
      await createBet({
        ...
        book: betForm.book,
        notes:
          `FBS model ${g.projectedSpread.toFixed(1)}` +
          (g.marketSpread === null
            ? ' · no book line'
...
${line}` : line);
  ...
  const byTeamName = useMemo(() => {
    const m = new Map<string, FbsTeamRating>();
    ...
    try {
      ...
      if (url.includes('/seed')) {
        const unmatched = json.unmatched?.length
          ...
          : '';
        setMessage(
          `Brad Powers: ${json.powersTeams} teams — ${json.inserted} inserted, ${json.refreshed} refreshed, ${json.reseeded} reseeded.${unmatched}`
        );
  ...
  const saveManualLine = async (gameId: string) => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/fbs/closing-line', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const submitManualRating = async (teamName: string) => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/fbs/manual-adjustment', {
  ...
  const rows = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (r) =>
          r.teamName.toLowerCase().includes(q) || r.powersName.toLowerCase().includes(q)
      );
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-6">
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={() => runAction('sync', '/api/fbs/calculate', { action: 'sync' })}
            >
            ...
            <button
              ...
              disabled={busy !== null}
              onClick={() =>
                runAction('seed', '/api/fbs/seed', {},
                  data && data.ratings.length > 0
                    ? 'Ratings already exist. This refreshes metadata (HFA/conference) from the latest Brad Powers set without touching adjusted ratings. Continue?'
                    : undefined
                )
              }
            >
              {busy === 'seed' ? 'Seeding…' : 'Seed Brad Powers'}
            </button>
            <button
              className={`${btnCls} flex-1 sm:flex-none`}
              ...
              onClick={() =>
                runAction(
                  ...
                  '/api/fbs/calculate',
                  { action: 'recalculate' },
                  'Reset all teams to their Brad Powers seed and replay every game chronologically?'
                )
              }
            >
        ...
        <div className="grid grid-cols-4 bg-slate-200/70 rounded-full p-0.5">
          {(['ratings', 'upcoming', 'futures', 'sos'] as const).map((v) => (
            <button
              key={v}
              onClick={() => setView(v)}
            ...
            >
              {v === 'ratings' ? 'Ratings' : v === 'upcoming' ? 'Upcoming' : v === 'futures' ? 'Futures' : 'SOS'}
            </button>
          ))}
        </div>
              ...
              <div>
                ...
                <div className="text-xs text-slate-500 mt-0.5">
                  ...
                  {crossInfo && crossInfo.count > 0 && (
                    <>
                      {' '}vs-FCS games use a {crossInfo.scaleOffset > 0 ? '+' : ''}
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div className="max-h-[45vh] overflow-y-auto divide-y divide-slate-100">
              {data!.unlinedGames.map((g) => {
                ...
                // positive. Mirrors processFbsGame's zero-sum split.
                    ...
                    {isOpen && (
                      <div className="px-3 sm:px-4 py-2 bg-slate-50/70 border-t border-slate-100">
                        <div className="text-[11px] text-slate-400 mb-1.5">
                          Powers seed {r.initialRating.toFixed(2)} {'\u2192'} now{' '}
