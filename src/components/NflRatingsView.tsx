...
// src/components/NflRatingsView.tsx
// NFL Ledger ratings (odds.day's market-driven power ratings: a market-implied
// preseason seed moved by closing lines) — shared by the admin page
// (/admin/nfl-ratings, admin=true: sync/seed/recalculate, manual lines,
// manual rating adjustments) and the public read-only page (/nfl).
...
const NFL_HISTORY: HistoryPanelConfig = {
  ...
  metrics: [
    ...
    { key: 'sosPct', label: 'SOS', hint: 'Median-team win % over the full slate — lower is harder', pct: true },
    { key: 'sosRemainingPct', label: 'Remaining SOS', hint: 'Median-team win % over unplayed games — lower is harder', pct: true },
  ...
  columns: [
    ...
    { key: 'sosPct', label: 'SOS', kind: 'pct' },
    { key: 'sosRemainingPct', label: 'Rem SOS', kind: 'pct' },
...
interface RatingsResponse {
  ...
  adjustments: NflGameAdjustment[];
  totalAdjustments: number;
  unlinedGames: NflClosingLine[];
  manualAdjustments: NflManualAdjustment[];
  pendingManualCount: number;
  error?: string;
}

type HistoryEntry =
  | { kind: 'game'; date: string; adj: NflGameAdjustment; isHome: boolean }
  | { kind: 'manual'; date: string; m: NflManualAdjustment };
        ...
        <div className="text-sm font-medium text-slate-800 truncate group-hover/team:underline">
          ...
          {warn && (
            <span
              ...
              title="No ESPN link — games for this team will be skipped. Add an override in src/lib/nfl/teamNames.ts and re-seed."
            >
              ⚠
            </span>
          )}
...
export default function NflRatingsView({ admin = false }: { admin?: boolean }) {
  ...
  const colorMap = useTeamColorMap('americanfootball_nfl');
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/nfl/ratings');
  ...
  const loadUpcoming = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/nfl/upcoming?days=14');
  ...
  // Pending NFL bets, so a game already wagered shows its ticket instead of
  ...
  const loadPendingBets = useCallback(async () => {
    try {
      ...
      for (const b of all) {
        if (b.league !== 'NFL' || b.status !== 'pending') continue;
  ...
  const submitBet = async (g: UpcomingGame) => {
    ...
    try {
      await createBet({
        ...
        book: betForm.book,
        notes:
          `NFL model ${g.projectedSpread.toFixed(1)}` +
          (g.marketSpread === null
            ? ' · no book line'
...
${line}` : line);
  ...
  const byTeamName = useMemo(() => {
    const m = new Map<string, NflTeamRating>();
  ...
  // Upcoming rows carry their own ESPN identity as a fallback.
    ...
    try {
      ...
      if (url.includes('/seed')) {
        const unmatched = json.unmatched?.length
          ...
          : '';
        const fit = json.fit
          ? ` Fit: ${json.fit.games} lined games, HFA ${json.fit.hfa}, RMSE ${json.fit.rmse}.`
          : json.refit === false
            ? ` (stored set from ${json.asOf ?? 'earlier'})`
            : '';
        const filled = json.filled?.length
          ? ` Sagarin fill: ${json.filled.map((f: { team: string }) => f.team).join(', ')}.`
          : '';
        setMessage(
  ...
  const saveManualLine = async (gameId: string) => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/nfl/closing-line', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const submitManualRating = async (teamName: string) => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/nfl/manual-adjustment', {
  ...
  const rows = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (r) =>
          r.teamName.toLowerCase().includes(q) || r.seedName.toLowerCase().includes(q)
      );
    }
      ...
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        <div className="space-y-3">
          <div>
            ...
            <h1 className="text-[22px] font-bold tracking-[-0.7px] text-slate-800">NFL Ledger Ratings</h1>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={() => runAction('sync', '/api/nfl/calculate', { action: 'sync' })}
            >
            ...
            <button
              ...
              disabled={busy !== null}
              onClick={() =>
                runAction('seed', '/api/nfl/seed', {},
                  data && data.ratings.length > 0
                    ? 'Ratings already exist. This refreshes metadata (division/logo) from the stored market seed without touching adjusted ratings. Continue?'
                    : undefined
                )
              }
            >
              {busy === 'seed' ? 'Seeding…' : 'Seed from Market'}
            </button>
            <button
              className={`${btnCls} flex-1 sm:flex-none`}
              disabled={busy !== null}
              onClick={() =>
                runAction(
                  'refit',
                  '/api/nfl/seed',
                  { refit: true, forceRefresh: true },
                  'Rebuild the market-implied seed from the lines the books post TODAY and reset every team to it? Preseason only — mid-season lines already reflect results. Follow with Recalculate if the ledger has games.'
                )
              }
            >
            ...
            <button
              ...
              onClick={() =>
                runAction(
                  ...
                  '/api/nfl/calculate',
                  { action: 'recalculate' },
                  'Reset all teams to their market seed and replay every game chronologically?'
                )
              }
            >
        ...
        {view === 'upcoming' && (
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            {upcomingLoading && upcoming === null ? (
              ...
              <div className="px-4 py-6 text-sm text-slate-500">
                No upcoming NFL games in the next 14 days.
              </div>
            ) : (
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-100 space-y-2">
                                ...
                                {betOpen && betForm && (
                                  <div className="space-y-1.5">
                                    ...
                                    <div className="text-[11px] text-slate-400">
                                      Saves to bet history as a pending NFL spread:{' '}
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div className="max-h-[45vh] overflow-y-auto divide-y divide-slate-100">
              {data!.unlinedGames.map((g) => {
                ...
                // positive. Mirrors processNflGame's zero-sum split.
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="px-4 py-6 text-sm text-slate-500">
              No ratings yet — run &quot;Seed from Market&quot; (fits every team from the season lines the books post).
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {rows.map((r, i) => {
                    ...
                    {isOpen && (
                      <div className="px-3 sm:px-4 py-2 bg-slate-50/70 border-t border-slate-100">
                        <div className="text-[11px] text-slate-400 mb-1.5">
                          Market seed {r.initialRating.toFixed(2)} {'\u2192'} now{' '}
